...
- **A country may fight several wars at once**, on either side — that is what makes a promise of protection worth anything, since a third country can strike an invader mid-war. Each war escrows, ticks, and ends on its own. The one overlap rejected is a second declaration on the *same* target. Anything acting on "the" war of a country is a bug: `/defend`, `/reinforce`, and `/surrender` take an `enemy` option and resolve it with `chooseWar`.
- **A country that falls takes its other wars with it.** Conquest cancels them inside the same transaction, returning each side's field force first, so the fallen country's armies abroad land in its stockpile a moment before the victor loots it.
